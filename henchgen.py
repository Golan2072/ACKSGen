...
class character():
        ...
        def __init__ (self, level,race): #input character level
                self.level=level
...
def test(level,race):
        ...
        """
        character1=character(level,race)
        #i=0
...
parser = argparse.ArgumentParser(description='Henchmen generation')
parser.add_argument('-l','--level', type=int, default=0,
                   help='choose level (default 0/NM)')
parser.add_argument('-r','--race',default="random",help='choose race (default random select)')

args = vars(parser.parse_args())

test(args['level'],args['race'])
